...
export default function DeliveryPage() {
  ...
  const [warningDraft, setWarningDraft] = useState<WarningDraftResponse | null>(null);
  ...
  const [selectedFellowForHistory, setSelectedFellowForHistory] = useState<any>(null);
  const [fellowWarnings, setFellowWarnings] = useState<Warning[]>([]);
  ...
  const fetchDashboard = async () => {
    try {
      const response = await deliveryAPI.getRiskDashboard(selectedCohortId || undefined);
  ...
  const handleDraftWarning = async (fellowId: string) => {
    ...
    try {
      ...
      setWarningDraft(data.draft);
      setWarningId(data.warning_id);
      setEditedMessage(data.draft.message || '');
  ...
  const handleViewHistory = async (fellow: any) => {
    setSelectedFellowForHistory(fellow);
    setHistoryModalOpen(true);
    setLoadingHistory(true);
    try {
      const response = await api.get(`/api/delivery/warnings/${fellow.id}`);
      setFellowWarnings(response.data);
    } catch (error) {
  ...
  const getLevelBadgeVariant = (level?: string) => {
    return level === 'final' ? 'danger' : 'warning';
  };

  // Sort fellows by risk level (critical first)
  const sortedFellows = dashboard?.fellows.sort((a, b) => {
    const riskOrder: Record<string, number> = { critical: 0, at_risk: 1, monitor: 2, on_track: 3 };
    const aRisk = a.risk_level || 'on_track';
    const bRisk = b.risk_level || 'on_track';
    return (riskOrder[aRisk] ?? 4) - (riskOrder[bRisk] ?? 4);
  }) || [];

  // Warning stats
  const totalWarnings = sortedFellows.reduce((sum, f) => sum + (f.warnings_count || 0), 0);
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-500">
            {totalWarnings} warning{totalWarnings !== 1 ? 's' : ''} issued
          </span>
        </div>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => handleViewHistory(fellow)}
                          >
        ...
        >
          {warningDraft && (
            <div className="space-y-6">
              ...
              {/* Key Points */}
              {warningDraft.key_points && warningDraft.key_points.length > 0 && (
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">Key Points</h4>
                  <ul className="list-disc list-inside space-y-1 bg-gray-50 p-4 rounded-lg">
                    {warningDraft.key_points.map((point, idx) => (
        ...
        <Modal
          ...
          title={`Warning History — ${selectedFellowForHistory?.name || ''}`}
          size="lg"
        >
          {loadingHistory ? (
            <div className="py-8 text-center text-gray-500">Loading warnings...</div>
          ) : fellowWarnings.length === 0 ? (
            <div className="py-8 text-center">
              <Shield className="mx-auto h-12 w-12 text-gray-400" />
              <h3 className="mt-2 text-sm font-medium text-gray-900">No warnings</h3>
              <p className="mt-1 text-sm text-gray-500">
                No warnings have been issued for this fellow.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {fellowWarnings.map((warning) => (
                <div key={warning.id} className="rounded-lg border border-gray-200 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Badge variant={getLevelBadgeVariant(warning.level)}>
                        {warning.level === 'final' ? 'Final Warning' : 'First Warning'}
                      </Badge>
                      {warning.issued_at ? (
                  ...
                  <div className="mb-3">
                    <p className="text-sm text-gray-700">
                      {warning.final_message || warning.draft_message || 'No message'}
                    </p>
                  </div>

                  {/* Concerns */}
                  {warning.concerns && warning.concerns.length > 0 && (
                    <div className="mb-2">
                      <p className="text-xs font-medium text-gray-500 mb-1">Concerns:</p>
                      <div className="flex flex-wrap gap-1">
                        {warning.concerns.map((c, i) => (
                  ...
                  {/* Requirements */}
                  {warning.requirements && warning.requirements.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-gray-500 mb-1">Requirements:</p>
                      <ul className="list-disc list-inside">
                        {warning.requirements.map((r, i) => (
                  ...
                  <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
                    {warning.issued_at && (
                      <span>Issued: {new Date(warning.issued_at).toLocaleString()}</span>
                    )}
                    {warning.acknowledged_at && (
                      <span>Acknowledged: {new Date(warning.acknowledged_at).toLocaleString()}</span>
                    )}
                  </div>
                </div>
              ))}
